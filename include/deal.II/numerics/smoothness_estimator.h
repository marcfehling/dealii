...
namespace SmoothnessEstimator
{
  namespace Legendre
  {
     ...
     *
     * In one dimension, the finite element solution on the reference element
     * with polynomial degree $p$ can be written as
     * @f[
     *    u_h(\hat x) = \sum_{j=0}^{p} a_j P_j(\hat x)
     * @f]
     * where $\{P_j(x)\}$ are Legendre polynomials. The decay of the
     * coefficients is estimated by performing the linear regression fit of
     * @f[
     ...
     * @f]
     * for $j=0,..,p$. The rate of the decay $\sigma$ can be used to estimate
     * the smoothness. For example, one strategy to implement hp-refinement
     ...
     * coefficients $a_j \;\; j=0\dots p$ for linear regression in each
     ...
     * there are less than two non-zero coefficients for a coordinate direction,
     * this direction will be skipped. If all coefficients are zero, the
     * returned value for this cell will be zero (i.e. corresponding to the
     * $\sigma=\infty$).
     ...
     * deal.II library @cite davydov2017hp .
     *
     * @ingroup numerics
  ...
  namespace Fourier
  {
     ...
     * matrix from the Fourier expansion of each shape function. We use the
     * class FESeries::Fourier to determine all coefficients $U_{\bf k}$.
     *
     ...
     * regularity estimates $s$ will suffice as our smoothness indicators and
