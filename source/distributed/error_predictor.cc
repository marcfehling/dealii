...
namespace parallel
{
  namespace distributed
  {
    ...
    template <int dim, int spacedim>
    void
    ErrorPredictor<dim, spacedim>::prepare_for_serialization(
      const Vector<float> &in,
      const double         gamma_p,
      const double         gamma_h,
      const double         gamma_n)
    {
      std::vector<const Vector<float> *> all_in(1, &in);
      prepare_for_serialization(all_in, gamma_p, gamma_h, gamma_n);
    }
    ...
    template <int dim, int spacedim>
    void
    ErrorPredictor<dim, spacedim>::deserialize(Vector<float> &in)
    {
      std::vector<Vector<float> *> all_in(1, &in);
      deserialize(all_in);
    }
    ...
    template <int dim, int spacedim>
    void
    ErrorPredictor<dim, spacedim>::deserialize(
      std::vector<Vector<float> *> &all_in)
    {
      register_data_attach();

      // this makes unpack() happy
      error_indicators.resize(all_in.size());
    ...
    ErrorPredictor<dim, spacedim>::unpack(std::vector<Vector<float> *> &all_out)
    {
      ...
      for (typename std::vector<Vector<float> *>::iterator it = all_out.begin();
           it != all_out.end();
           ++it)
        ...
        switch (status)
          {
            ...
            case parallel::distributed::Triangulation<dim,
                                                      spacedim>::CELL_REFINE:
              {
                ...
                *predicted_error_it =
                  (**estimated_error_it)[cell->active_cell_index()] *
                  (gamma_h * std::pow(.5, dim + future_fe_degree));
            ...
            case parallel::distributed::Triangulation<dim,
                                                      spacedim>::CELL_COARSEN:
              {
                ...
                float sum_of_predicted_errors = 0.;
                ...
                for (unsigned int child_index = 0;
                     ...
                     ++child_index)
                  {
                    ...
                    sum_of_predicted_errors += predicted_error;
                  }
                *predicted_error_it = std::move(sum_of_predicted_errors);
